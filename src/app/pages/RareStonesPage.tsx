...
export default function RareStonesPage() {
  ...
  useEffect(() => {
    const fetchGems = async () => {
      try {
        const gems = await api.getAllGems('Rare Stones');
        ...
        console.error('Error fetching rare stones:', error);
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          {loading ? (
            <div className="py-20 text-center">
              <p className="text-gray-600 font-body">Loading rare stones...</p>
            </div>
          ) : products.length === 0 ? (
            <div className="py-20 text-center">
              <p className="text-gray-600 font-body">No rare stones available at the moment.</p>
            </div>
          ) : (
